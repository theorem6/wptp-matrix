...
function resolveChrysalisEmitScript(): string | null {
  const root = process.env.CHRYSALIS_ROOT;
  if (root) return join(root, "scripts", "emit-webir-bundle-hono.mjs");
  return null;
}
...
export function composeOpenApiIrHono(
  ...
  if (options?.chrysalisRoot) process.env.CHRYSALIS_ROOT = options.chrysalisRoot;
  ...
  if (options?.chrysalisRoot !== undefined) {
    if (prevRoot === undefined) delete process.env.CHRYSALIS_ROOT;
    else process.env.CHRYSALIS_ROOT = prevRoot;
  }
